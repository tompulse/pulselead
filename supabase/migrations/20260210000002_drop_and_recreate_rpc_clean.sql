-- NETTOYAGE COMPLET ET RECRÉATION DE LA FONCTION RPC
-- Résout le problème "function is not unique"
...
DROP FUNCTION IF EXISTS public.get_nouveaux_sites_filter_counts_dynamic(text[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.get_nouveaux_sites_filter_counts_dynamic(text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.get_nouveaux_sites_filter_counts_dynamic();
DROP FUNCTION IF EXISTS public.get_nouveaux_sites_filter_counts_dynamic CASCADE;

-- 2. CRÉER UNE SEULE VERSION PROPRE AVEC LES VRAIES COLONNES
...
BEGIN
  WITH filtered_base AS (
    SELECT 
      ...
      code_naf,
      -- Normaliser le département avec gestion codes postaux 4/5 caractères
      CASE 
    ...
    WHERE 
      ...
      -- Filtre départements (normalisation 4/5 caractères)
  ...
  departments_agg AS (
    ...
    WHERE dept IS NOT NULL 
      AND dept != ''
      AND dept ~ '^[0-9]+$'
      -- IMPORTANT: Ne plus exclure 97 et 98 (départements d'outre-mer)
    GROUP BY dept
  ),
...
SELECT 'Fonction recréée avec succès !' as status;
