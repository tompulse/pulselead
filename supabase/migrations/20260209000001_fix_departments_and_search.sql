-- Fix RPC function to handle postal codes with/without leading zero and add postal code search
...
BEGIN
  WITH filtered_base AS (
    ...
    WHERE 
      archived != true
      AND (p_naf_sections IS NULL OR naf_section = ANY(p_naf_sections))
      AND (p_naf_divisions IS NULL OR naf_division = ANY(p_naf_divisions))
      AND (p_departments IS NULL OR 
           -- Gérer les codes postaux avec ou sans zéro initial
           CASE 
             WHEN LENGTH(code_postal) = 4 THEN '0' || LEFT(code_postal, 1)
             WHEN LENGTH(code_postal) >= 5 THEN LEFT(code_postal, 2)
             ELSE NULL
      ...
      AND (p_categories_juridiques IS NULL OR 
           EXISTS (
             SELECT 1 FROM unnest(p_categories_juridiques) AS cat 
             WHERE categorie_juridique LIKE cat || '%'
           )
      )
      AND (p_types_etablissement IS NULL OR 
           (CASE WHEN est_siege = true THEN 'siege' ELSE 'site' END) = ANY(p_types_etablissement))
      AND (p_search_query IS NULL OR p_search_query = '' OR (
        nom ILIKE '%' || p_search_query || '%' OR
        entreprise ILIKE '%' || p_search_query || '%' OR
        ville ILIKE '%' || p_search_query || '%' OR
        adresse ILIKE '%' || p_search_query || '%' OR
        siret ILIKE '%' || p_search_query || '%' OR
        code_naf ILIKE '%' || p_search_query || '%' OR
        code_postal ILIKE '%' || p_search_query || '%'
      ))
  ),
  naf_sections_agg AS (
