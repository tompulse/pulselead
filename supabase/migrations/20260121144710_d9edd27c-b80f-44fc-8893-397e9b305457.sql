-- Update admin analytics functions to exclude admin accounts from KPIs

-- Recreate get_admin_tournee_stats to exclude admin accounts
...
BEGIN
  ...
  -- Get admin user IDs to exclude (tom.iolov@hotmail.fr and tomiolovpro@gmail.com)
  SELECT array_agg(id) INTO admin_user_ids
  FROM auth.users 
  WHERE email IN ('tom.iolov@hotmail.fr', 'tomiolovpro@gmail.com');
  ...
  FROM tournees
  WHERE user_id != ALL(COALESCE(admin_user_ids, ARRAY[]::uuid[]));

  RETURN result;
END;
$$;

-- Recreate get_admin_crm_stats to exclude admin accounts
...
BEGIN
  ...
  FROM lead_statuts 
  WHERE statut = 'gagne' 
  AND user_id != ALL(COALESCE(admin_user_ids, ARRAY[]::uuid[]));
...
-- Recreate get_admin_users_activity to exclude admin accounts
...
BEGIN
  ...
  FROM (
    ...
    WHERE u.id != ALL(COALESCE(admin_user_ids, ARRAY[]::uuid[]))
...
-- Recreate get_admin_timeseries to exclude admin accounts
